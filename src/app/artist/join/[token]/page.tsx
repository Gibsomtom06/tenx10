...
export default async function ArtistJoinPage({
  ...
  const { data: inviteRaw } = await (supabase as any)
    .from('artist_invites')
  ...
  const artist = invite.artists as { id: string; stage_name: string | null; name: string; genre: string | null; email: string | null } | null
